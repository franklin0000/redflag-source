...
def open_yandex_search(image_path):
    """Abre Yandex Images con filtros específicos."""
    print("Abriendo búsqueda profunda en Yandex...")
    yandex_url = "https://yandex.com/images/search"
    
    # Nota: La subida automática via requests a veces requiere headers complejos.
    # Como fallback, abrimos la búsqueda con el query sugerido.
    search_query = "onlyfans+porn+escort+nsfw"
    # Intentamos construir la URL de búsqueda por imagen si fuera posible, 
    # pero suele requerir subir el archivo primero a un hosting.
    # Por ahora abrimos la página de búsqueda para que el usuario suba o use el query.
    webbrowser.open(f"{yandex_url}?text={search_query}")
